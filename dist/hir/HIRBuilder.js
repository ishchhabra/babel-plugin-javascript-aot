...
import { ArrayExpressionInstruction, BinaryExpressionInstruction, BranchTerminal, CallExpressionInstruction, createBlock, createIdentifier, createPlace, ExpressionStatementInstruction, FunctionDeclarationInstruction, HoleInstruction, JumpTerminal, LiteralInstruction, LoadGlobalInstruction, LoadLocalInstruction, makeInstructionId, MemberExpressionInstruction, ReturnTerminal, SpreadElementInstruction, StatementInstruction, StoreLocalInstruction, UnaryExpressionInstruction, UnsupportedNodeInstruction, } from "../ir";
/**
...
export class HIRBuilder {
    ...
    #buildBindings(bindingsPath) {
                ...
                switch (path.node.type) {
                    ...
                    case "VariableDeclaration": {
                        ...
                        for (const declaration of path.node.declarations) {
                            ...
                            const identifier = createIdentifier(this.environment);
                            this.#registerDeclarationName(declaration.id.name, identifier.declarationId, bindingsPath);
                            // Rename the variable name in the scope to the temporary place.
        ...
        if (bindingsPath.isFunctionDeclaration()) {
            ...
            for (const paramPath of paramPaths) {
                ...
                const identifier = createIdentifier(this.environment);
                this.#registerDeclarationName(paramPath.node.name, identifier.declarationId, bindingsPath);
                // Rename the variable name in the scope to the temporary place.
    ...
    #buildVariableDeclaration(statementPath) {
        ...
        for (const declaration of declarations) {
            ...
            if (declarationId === undefined) {
                throw new Error(`Variable accessed before declaration: ${id.node.name}`);
            }
            const lvalPlace = this.#getLatestDeclarationPlace(declarationId);
            if (lvalPlace === undefined) {
                throw new Error(`Unable to find the place for ${id.node.name} (${declarationId})`);
            }
