...
import { CallGraphBuilder } from '../analysis/CallGraph.js';
...
class FunctionInliningPass extends BaseOptimizationPass {
    ...
    moduleIR;
    callGraph;
    constructor(functionIR, moduleIR, callGraph) {
        super(functionIR);
    ...
    step() {
        ...
        for (const [, block] of this.functionIR.blocks) {
            for (const [index, instr] of block.instructions.entries()) {
                ...
                const functionIRId = CallGraphBuilder.resolveFunctionIRId(instr, this.callGraph.declarations);
    ...
    inlineFunctionIR(index, callExpressionBlock, funcIR, moduleIR) {
        if (funcIR.blocks.size > 1) {
