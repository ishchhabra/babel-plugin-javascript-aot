...
class SSAEliminator {
    ...
    #insertPhiDeclaration(phi) {
        ...
        const declaration = this.moduleUnit.environment.declToPlaces.get(declarationId)?.[0];
        ...
        const identifier = createIdentifier(this.moduleUnit.environment, declarationId);
    ...
    #insertPhiCopies(phi) {
        for (const [blockId, place] of phi.operands) {
            ...
            const copyPlace = createPlace(createIdentifier(this.moduleUnit.environment), this.moduleUnit.environment);
