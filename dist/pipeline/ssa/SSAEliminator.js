...
class SSAEliminator {
    ...
    #insertPhiDeclaration(phi) {
        ...
        const identifier = this.moduleIR.environment.createIdentifier(phi.place.identifier.declarationId);
        ...
        const instruction = new StoreLocalInstruction(instructionId, place, undefined, phi.place, declarationPlace, "let");
    ...
    #insertPhiCopies(phi) {
        for (const [blockId, place] of phi.operands) {
            ...
            const loadPlace = this.moduleIR.environment.createPlace(this.moduleIR.environment.createIdentifier());
            ...
            const copyPlace = this.moduleIR.environment.createPlace(this.moduleIR.environment.createIdentifier(phi.place.identifier.declarationId));
            ...
            const exprPlace = this.moduleIR.environment.createPlace(this.moduleIR.environment.createIdentifier());
    ...
    #replacePhiUsages(phi) {
        ...
        for (const [blockId, block] of this.functionIR.blocks) {
            ...
            block.instructions = block.instructions.map((instruction) => instruction.rewrite(values));
        }
    }
}
