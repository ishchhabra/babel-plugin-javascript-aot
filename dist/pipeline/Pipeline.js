import { CommonJSExportCollectorPass } from '../frontend/passes/CommonJSExportCollectorPass.js';
import { CallGraph } from './analysis/CallGraph.js';
        ...
        for (const moduleName of this.projectUnit.postOrder.toReversed()) {
            ...
            for (const functionIR of moduleIR.functions.values()) {
                ...
                if (this.options.enableOptimizer) {
                    const optimizerResult = new Optimizer(functionIR, moduleIR, callGraph, ssaBuilderResult, this.projectUnit, this.options, context).run();
