...
function buildObjectMethod(nodePath, functionBuilder, moduleBuilder) {
    ...
    const bodyIR = new FunctionIRBuilder(bodyPath, functionBuilder.environment, moduleBuilder).build();
    ...
    functionBuilder.currentBlock.instructions.push(new ObjectMethodInstruction(methodInstructionId, methodPlace, nodePath, keyPlace, paramPlaces, bodyIR, nodePath.node.computed, nodePath.node.generator, nodePath.node.async, nodePath.node.kind));
    return methodPlace;
}
