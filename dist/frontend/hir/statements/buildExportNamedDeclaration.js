...
function buildExportNamedDeclaration(nodePath, functionBuilder, moduleBuilder, environment) {
    ...
    if (declarationPath.hasNode()) {
        ...
        const instruction = new ExportNamedDeclarationInstruction(makeInstructionId(environment.nextInstructionId++), place, nodePath, [], declarationPlace);
        ...
        const identifier = environment.createIdentifier();
        const place = environment.createPlace(identifier);
        const instruction = new ExportNamedDeclarationInstruction(makeInstructionId(environment.nextInstructionId++), place, nodePath, exportSpecifierPlaces, undefined);
        functionBuilder.addInstruction(instruction);
        return place;
    }
}
