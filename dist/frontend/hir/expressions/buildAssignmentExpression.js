...
function buildIdentifierAssignmentLeft(leftPath, nodePath, functionBuilder, environment) {
    ...
    const place = environment.createPlace(identifier);
    functionBuilder.addInstruction(new BindingIdentifierInstruction(createInstructionId(environment), place, nodePath, identifier.name));
...
function buildMemberExpressionAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder, environment) {
    ...
    const storePropertyInstruction = new StorePropertyInstruction(createInstructionId(environment), storePropertyPlace, nodePath, objectPlace, property, loadLocalPlace);
    const expressionStatementPlace = environment.createPlace(environment.createIdentifier());
    const expressionStatementInstruction = new ExpressionStatementInstruction(createInstructionId(environment), expressionStatementPlace, nodePath, storePropertyPlace);
    return {
        place,
        instructions: [
...
function buildArrayPatternAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder, environment) {
    ...
    const identifier = environment.createIdentifier();
    const place = environment.createPlace(identifier);
    functionBuilder.addInstruction(new ArrayPatternInstruction(createInstructionId(environment), place, nodePath, elementPlaces));
    return { place, instructions };
}
function buildObjectPatternAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder, environment) {
    ...
    const propertyPlaces = propertyPaths.map((propertyPath) => {
        if (propertyPath.isObjectProperty()) {
            ...
            const identifier = environment.createIdentifier();
            const place = environment.createPlace(identifier);
            functionBuilder.addInstruction(new ObjectPropertyInstruction(createInstructionId(environment), place, nodePath, keyPlace, valuePlace, propertyPath.node.computed, propertyPath.node.shorthand));
            return place;
        }
    ...
    const identifier = environment.createIdentifier();
    const place = environment.createPlace(identifier);
    functionBuilder.addInstruction(new ObjectPatternInstruction(createInstructionId(environment), place, leftPath, propertyPlaces));
    return { place, instructions };
}
