...
function buildAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder) {
    if (leftPath.isIdentifier()) {
        ...
        return buildObjectPatternAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder);
    }
...
function buildObjectPatternAssignmentLeft(leftPath, nodePath, functionBuilder, moduleBuilder) {
    const instructions = [];
    ...
    const propertyPlaces = propertyPaths.map((propertyPath) => {
        if (propertyPath.isObjectProperty()) {
            ...
            const { place: valuePlace, instructions: valueInstructions } = buildAssignmentLeft(valuePath, nodePath, functionBuilder, moduleBuilder);
            instructions.push(...valueInstructions);
            const identifier = createIdentifier(functionBuilder.environment);
            const place = createPlace(identifier, functionBuilder.environment);
            functionBuilder.addInstruction(new ObjectPropertyInstruction(createInstructionId(functionBuilder.environment), place, nodePath, keyPlace, valuePlace, propertyPath.node.computed, propertyPath.node.shorthand));
            return place;
        }
    ...
    const identifier = createIdentifier(functionBuilder.environment);
    const place = createPlace(identifier, functionBuilder.environment);
    functionBuilder.addInstruction(new ObjectPatternInstruction(createInstructionId(functionBuilder.environment), place, leftPath, propertyPlaces));
    return { place, instructions };
}
