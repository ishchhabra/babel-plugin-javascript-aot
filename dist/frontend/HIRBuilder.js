...
import { makeInstructionId, ExportNamedDeclarationInstruction, ExportDefaultDeclarationInstruction, ExportSpecifierInstruction, ExpressionStatementInstruction, StatementInstruction, FunctionDeclarationInstruction, ImportDeclarationInstruction, ImportSpecifierInstruction, StoreLocalInstruction, UnsupportedNodeInstruction, ArrayExpressionInstruction, BinaryExpressionInstruction, CallExpressionInstruction, IdentifierInstruction, LoadGlobalInstruction, LoadLocalInstruction, LogicalExpressionInstruction, MemberExpressionInstruction, ObjectExpressionInstruction, ObjectMethodInstruction, ObjectPropertyInstruction, UnaryExpressionInstruction, LiteralInstruction, ArrayPatternInstruction, JSXElementInstruction, JSXFragmentInstruction, JSXTextInstruction, HoleInstruction, SpreadElementInstruction } from './ir/Instruction.js';
...
class HIRBuilder {
    ...
    #buildExportNamedDeclaration(statementPath) {
        ...
        if (declarationPath.hasNode()) {
            ...
            const identifier = createIdentifier(this.environment);
            const place = createPlace(identifier, this.environment);
            const instruction = new ExportNamedDeclarationInstruction(makeInstructionId(this.environment.nextInstructionId++), place, statementPath, [], declarationPlace);
            this.currentBlock.instructions.push(instruction);
            this.exportToInstructions.set(identifier.name, instruction);
            return place;
        }
        else {
            const exportSpecifierPlaces = specifiersPath.map((specifierPath) => this.#buildExportSpecifier(specifierPath));
            const identifier = createIdentifier(this.environment);
            const place = createPlace(identifier, this.environment);
            const instruction = new ExportNamedDeclarationInstruction(makeInstructionId(this.environment.nextInstructionId++), place, statementPath, exportSpecifierPlaces, undefined);
            this.currentBlock.instructions.push(instruction);
            this.exportToInstructions.set(identifier.name, instruction);
            return place;
        }
    ...
    #buildExportDefaultDeclaration(statementPath) {
        ...
        const instruction = new ExportDefaultDeclarationInstruction(instructionId, place, statementPath, declarationPlace);
        this.currentBlock.instructions.push(instruction);
        this.exportToInstructions.set("default", instruction);
        return place;
    }
    #buildExportSpecifier(specifierPath) {
        specifierPath.assertExportSpecifier();
        const localPath = specifierPath.get("local");
        const localPlace = this.#buildNode(localPath);
        ...
        const identifier = createIdentifier(this.environment);
        const place = createPlace(identifier, this.environment);
        const instruction = new ExportSpecifierInstruction(makeInstructionId(this.environment.nextInstructionId++), place, specifierPath, localPlace, exportedName);
        this.currentBlock.instructions.push(instruction);
    ...
    #buildImportDeclaration(statementPath) {
        ...
        const specifiersPath = statementPath.get("specifiers");
        const specifierPlaces = specifiersPath.map((specifierPath) => this.#buildImportSpecifier(specifierPath));
        const identifier = createIdentifier(this.environment);
        const place = createPlace(identifier, this.environment);
        this.currentBlock.instructions.push(new ImportDeclarationInstruction(makeInstructionId(this.environment.nextInstructionId++), place, statementPath, sourceValue, resolvedSourceValue, specifierPlaces));
        this.importToPlaces.set(resolvedSourceValue, place);
        return place;
    }
    #buildImportSpecifier(specifierPath) {
        if (!specifierPath.isImportSpecifier()) {
            throw new Error(`Unsupported import specifier type: ${specifierPath.type}`);
        }
        ...
        const identifier = createIdentifier(this.environment);
        const place = createPlace(identifier, this.environment);
        this.currentBlock.instructions.push(new ImportSpecifierInstruction(makeInstructionId(this.environment.nextInstructionId++), place, specifierPath, importedPlace, localPlace));
        return place;
    }
     ...
     */
    #buildIdentifier(expressionPath) {
    ...
    #buildBindingIdentifier(expressionPath) {
        ...
        this.currentBlock.instructions.push(new IdentifierInstruction(makeInstructionId(this.environment.nextInstructionId++), place, expressionPath, name));
        return place;
    }
    #buildReferencedIdentifier(expressionPath) {
        const name = expressionPath.node.name;
        const declarationId = this.#getDeclarationId(name, expressionPath);
        const identifier = createIdentifier(this.environment);
        const place = createPlace(identifier, this.environment);
        ...
        if (declarationId === undefined) {
            ...
            this.currentBlock.instructions.push(new LoadGlobalInstruction(instructionId, place, expressionPath, name, binding?.kind === "module" ? "import" : "builtin", binding?.kind === "module"
            ...
            this.currentBlock.instructions.push(new LoadLocalInstruction(instructionId, place, expressionPath, declarationPlace));
        }
        return place;
    }
