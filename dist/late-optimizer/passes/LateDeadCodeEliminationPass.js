import { CopyInstruction, StoreLocalInstruction, } from "../../ir";
import { BaseOptimizationPass } from "../OptimizationPass";
/**
    ...
    step() {
        ...
        for (const blockId of this.postOrder) {
            ...
            try {
                ...
                console.error(`Error eliminating dead code in block ${blockId}:`, error);
            }
        }
        return { changed };
    }
    eliminateDeadCodeInBlock(block, usedPlaceIds) {
        ...
        for (const instr of instrs) {
            for (const place of instr.getReadPlaces()) {
                if (place.identifier.id === 12) {
                    console.log(`instr.constructor.name: ${instr.constructor.name} is reading ${place.identifier.name}`);
                }
        ...
        for (const instr of instrs) {
            if (this.shouldKeepInstruction(instr, usedPlaceIds)) {
                ...
                console.log("Removing instruction", instr.place.identifier.name, instr.constructor.name);
