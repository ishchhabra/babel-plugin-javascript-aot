...
class Environment {
    ...
    registerDeclaration(declarationId, blockId, placeId) {
    ...
    getLatestDeclaration(declarationId) {
        const placeIds = this.declToPlaces.get(declarationId) ?? [];
        return placeIds[placeIds.length - 1];
    }
    registerDeclarationInstruction(declarations, instruction) {
